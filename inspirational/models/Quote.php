...
class Quote{
    ...
    public function fetchAllQuotes(){
        $query="SELECT quotes.id, quotes.body, quotes.date, users.firstName, users.lastName, categories.name As categoryName FROM quotes
                LEFT JOIN categories ON quotes.category_id = categories.id LEFT JOIN users ON quotes.user_id = users.i";
    ...
    public function fetchOneQuote($parameter){
        $query="SELECT quotes.id, quotes.body, quotes.date, users.firstName, users.lastName, categories.name As categoryName FROM quotes
        LEFT JOIN categories ON quotes.category_id = categories.id LEFT JOIN users ON quotes.user_id = users.id WHERE quotes.id = ?";
    ...
    public function fetchRandomQuotes($limit){
        $query="SELECT quotes.id, quotes.body, quotes.date, users.firstName, users.lastName, categories.name As categoryName FROM quotes
        LEFT JOIN categories ON quotes.category_id = categories.id LEFT JOIN users ON quotes.user_id = users.id  ORDER BY RAND() 
        LIMIT $limit";
    ...
    public function fetchUsersQuote($id){
        $query="SELECT quotes.id, quotes.body, quotes.date, users.firstName, users.lastName, categories.name As categoryName FROM quotes
        LEFT JOIN categories ON quotes.category_id = categories.id LEFT JOIN users ON quotes.user_id = users.id WHERE users.id = '$id'
